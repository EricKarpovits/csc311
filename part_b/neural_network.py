...
class AutoEncoder(nn.Module):
    ...
    def __init__(self, num_question, dim_first, code_vect):
        ...
        self.g = nn.Linear(num_question, dim_first)
        self.encode1 = nn.Linear(dim_first, code_vect)

        self.decode1 = nn.Linear(code_vect, dim_first)
        self.h = nn.Linear(dim_first, num_question)
...
def main():
    ...
    lr_list = [0.001, 0.05, 0.005]
    num_epoch_list = [10, 30, 50, 100]

    tune_iter = 1
    tuned_params = [0, 0, 0, 0, 0] # [k, lr, num_epoch, valid_acc]
    param_combs = len(k_list) * len(lr_list) * len(num_epoch_list)
    for dim_first in k_list:
        for dim_second in k_list:
            for curr_lr in lr_list:
                for curr_epoch in num_epoch_list:
                    print("############### Trial {}/{}: k = {}, lr = {}, num_epoch = {} ###############".format(tune_iter, param_combs, dim_first, curr_lr, curr_epoch))

                    curr_model = AutoEncoder(train_matrix.shape[1], dim_first)
                    ...
                    if curr_valid_acc > tuned_params[-1]:
                        tuned_params = [dim_first, curr_lr, curr_epoch, curr_valid_acc]
