    ...
    for curr_k in k_list:
        for curr_lr in lr_list:
            for curr_epoch in num_epoch_list:
                print(
                    "############### Trial {}/{}: k = {}, lr = {}, num_epoch = {} ###############".format(
                        tune_iter, param_combs, curr_k, curr_lr, curr_epoch))

                curr_model = AutoEncoder(train_matrix.shape[1], curr_k)
                curr_valid_acc = train(curr_model, curr_lr, lamb, train_matrix, zero_train_matrix,
                                       valid_data, curr_epoch, plot=False)
